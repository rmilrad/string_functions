	...
	if (eligible == 1) {
		for (int i = 0; i < (current_string.size()); i++) { //starting at beginning of current_string and loop till last index
			for (int j = 0; j < old_substring.size(); j++) { //starting at beginning of old_substring
				if (i == current_string.size()-1) { //if current_string index is beyond length, break loop
					break;
				}
			...
			if (loc != -1) { //if a match has been found and location has been set
				if (new_substring.size() != 0) {
					...
					locEnd = loc + old_substring.size() - 1;
				}

				if (new_substring.size() > old_substring.size()) {
					diff = new_substring.size() - old_substring.size();
				}
				else {
					diff = old_substring.size() - new_substring.size();
				}
				...
				else if (diff == old_substring.size()) {
				...
				i += (new_substring.size()); //set i to a value to pick up at the index after the last element of the new_substring
