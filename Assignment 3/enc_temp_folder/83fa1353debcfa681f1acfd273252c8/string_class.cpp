...
void string_class::replace_all(const string old_substring, const string new_substring) {
	...
	if (eligible == true) {
		int locEnd, diff, loc=0;
		for (int i = 0; i < current_string.size(); i++) {
			for (int j = 0; j < old_substring.size(); j++) {
				if (current_string[i + j] != old_substring[j]) {
					loc = -1;
					break;
				}
				else {
					if (current_string[i + j] == old_substring[j] && j == old_substring.size() - 1) {
						loc = i;
					}
				}
			}
			if (loc != -1) {
				...
				if (old_substring.size() == new_substring.size()) {
					for (int t = 0; t < new_substring.size(); t++) {
						current_string[loc + t] = new_substring[t];
					}
				}
				else {
					for (int k = (current_string.size() - 1 + new_substring.size() - old_substring.size()); k > locEnd; k--) {
